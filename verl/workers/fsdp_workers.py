...
class FSDPWorker(Worker):
    ...
    def _process_aug_multi_modal_inputs(self, data: DataProto):
        if "aug_multi_modal_data" not in data.non_tensor_batch:
            raise ValueError("aug_multi_modal_data is not found in the data.")
        ...
        aug_multi_modal_inputs = np.array(batch_multi_modal_inputs, dtype=object)
    ...
    # @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    # def compute_log_probs_aug(self, data: DataProto):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_log_probs_probe(self, data: DataProto):
        assert self._has_actor
        probe_data = data.pop(
            batch_keys=["input_ids_probe", "attention_mask_probe", "position_ids_probe","responses_probe"],
            non_tensor_batch_keys=["raw_prompt_ids_probe", "multi_modal_data_probe","uid"],
        )
        ...
        rename_map = {
            ...
            "responses_probe": "responses",
        }
        for old_k, new_k in rename_map.items():
            if old_k in probe_data.batch.keys():
                probe_data.batch.rename_key_(old_k, new_k)
        ...
        for old_k, new_k in rename_map_nt.items():
            if old_k in probe_data.non_tensor_batch:
                probe_data.non_tensor_batch[new_k] = probe_data.non_tensor_batch.pop(old_k)
        
        self._process_multi_modal_inputs(probe_data)
        probe_data = probe_data.to(torch.cuda.current_device())

        if self._use_param_offload:
            load_fsdp_model(self.fsdp_module)
        probe_data.meta_info = data.meta_info
        # compute aug_log_probs
        probe_data.meta_info["temperature"] = self.config.rollout.temperature
        # perform recompute log_prob
        print("##########################compute_log_probs_probe##########################")
        with self.ulysses_sharding_manager:
            probe_data = self.ulysses_sharding_manager.preprocess_data(probe_data)
            output = self.actor.compute_log_prob(data=probe_data)
            output = DataProto.from_dict(
                tensors={"old_log_probs_probe": output}, meta_info={"temperature": self.config.rollout.temperature}
            )
            output = self.ulysses_sharding_manager.postprocess_data(output)
