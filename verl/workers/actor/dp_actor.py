...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto) -> Dict[str, Any]:
        ...
        non_tensor_select_keys = ["multi_modal_inputs"]
        if "advantages_probe" in data.batch.keys():
            select_keys.extend(["responses_probe", "input_ids_probe","attention_mask_probe","position_ids_probe"])
            select_keys.extend(["old_log_probs_probe", "advantages_probe"])
            non_tensor_select_keys.extend(["multi_modal_inputs_probe"])
        # for contrastive kl
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    if "advantages_probe" in model_inputs:  
                        # 构造 probe 输入：非 probe 字段 + 覆盖的 probe 字段（去掉后缀）
                        model_inputs_probe = {
                            **{k: v for k, v in model_inputs.items() if not k.endswith("_probe")},
                            **{k[:-6]: v for k, v in model_inputs.items() if k.endswith("_probe")},
                        }

                        # 提取常用张量
                        attention_mask_probe = model_inputs["attention_mask_probe"]
                        resp_len_probe = model_inputs["responses_probe"].size(1)
                        response_mask_probe = attention_mask_probe[:, -resp_len_probe:]

                        # 前向与损失
                        log_probs_probe = self._forward_micro_batch(model_inputs_probe, temperature=temperature)
                        pg_loss_probe, pg_metrics_probe = compute_policy_loss(
                            old_log_probs=model_inputs["old_log_probs_probe"],
                            log_probs=log_probs_probe,
                            advantages=model_inputs["advantages_probe"],
                            response_mask=response_mask_probe,
                            clip_ratio_low=self.config.clip_ratio_low,
                            clip_ratio_high=self.config.clip_ratio_high,
                            clip_ratio_dual=self.config.clip_ratio_dual,
                            loss_avg_mode=self.config.loss_avg_mode,
                        ...
                        if self.config.enable_stage1:
                            pg_loss = pg_loss_probe 
                        else:
                            pg_loss = pg_loss + pg_loss_probe * self.config.probe_loss_coef
                        # pg_loss_probe = pg_loss_probe * self.config.probe_loss_coef
                        # pg_loss_probe = pg_loss_probe/gradient_accumulation
                        # pg_loss_probe.backward()
                        append_to_dict(metrics, {
                            "actor/pg_probe_loss": pg_loss_probe.detach().item(),
                            "actor/pg_probe_clipfrac_higher": pg_metrics_probe["pg_clipfrac_higher"],
                            "actor/pg_probe_clipfrac_lower": pg_metrics_probe["pg_clipfrac_lower"],
                            "actor/pg_probe_entropy_loss_est": pg_metrics_probe["entropy_loss"],
                            "actor/pg_probe_ppo_kl": pg_metrics_probe["ppo_kl"],
                        })
