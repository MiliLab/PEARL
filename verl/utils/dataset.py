...
class RLHFDataset(Dataset):
    ...
    def _build_messages_probe(self, example: Dict[str, Any]) -> List[Dict[str, Any]]:
        prompt_str: str = example["probe_problem"]
        if "<q1>" in prompt_str:
            prompt_str = prompt_str + """
                You are given a list of questions embedded in <q1>...</q1>, <q2>...</q2>, <q3>...</q3>, ...
                Answer the questions and put the answers in the box <a1>...</a1>, <a2>...</a2>, <a3>...</a3>, ...
                """
        else:
            prompt_str = prompt_str + "Answer the question and put the final answer in the box \\boxed{}."
    ...
    def __getitem__(self, index):
        ...
        messages = self._build_messages(example)
        if "probe_problem" in example:
            messages_probe = self._build_messages_probe(example)
            if self.image_key in example:
                prompt = self.processor.apply_chat_template(messages, add_generation_prompt=True, tokenize=False)
                prompt_probe = self.processor.apply_chat_template(messages_probe, add_generation_prompt=True, tokenize=False)
                ...
                model_inputs = self.processor(processed_images, [prompt], add_special_tokens=False, return_tensors="pt")
                model_inputs_probe = self.processor(processed_images, [prompt_probe], add_special_tokens=False, return_tensors="pt")
                input_ids = model_inputs.pop("input_ids")[0]
                attention_mask = model_inputs.pop("attention_mask")[0]
                input_ids_probe = model_inputs_probe.pop("input_ids")[0]
                attention_mask_probe = model_inputs_probe.pop("attention_mask")[0]
                example["multi_modal_data"] = {"images": images}
                example["multi_modal_data_probe"] = {"images": images}
            ...
            if self.processor is not None and "Qwen2VLImageProcessor" in self.processor.image_processor.__class__.__name__:
                ...
                position_ids_probe = get_rope_index(
                    self.processor,
                    input_ids=input_ids_probe,
                    image_grid_thw=model_inputs_probe.get("image_grid_thw", None),
                    video_grid_thw=model_inputs_probe.get("video_grid_thw", None),
                    second_per_grid_ts=model_inputs_probe.get("second_per_grid_ts", None),
                    attention_mask=attention_mask_probe,
                )  # (3, seq_length)
            else:
            ...
            input_ids_probe, attention_mask_probe, position_ids_probe = VF.postprocess_data(
                input_ids=input_ids_probe,
                attention_mask=attention_mask_probe,
                position_ids=position_ids_probe,
                max_length=self.max_prompt_length,
                pad_token_id=self.tokenizer.pad_token_id,
                left_pad=True,
                truncation=self.truncation,
            ...
            raw_prompt_ids_probe = self.tokenizer.encode(prompt_probe, add_special_tokens=False)
            ...
            example["input_ids_probe"] = input_ids_probe
            example["attention_mask_probe"] = attention_mask_probe
            example["position_ids_probe"] = position_ids_probe
            example["raw_prompt_ids_probe"] = raw_prompt_ids_probe
            example["ground_truth_probe"] = example.pop("probe_answer")
        else:
            if self.image_key in example:
                prompt = self.processor.apply_chat_template(messages, add_generation_prompt=True, tokenize=False)
                images = example.pop(self.image_key)
